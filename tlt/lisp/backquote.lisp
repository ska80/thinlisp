...
(defun read-backquote (stream char)
  (declare (ignore char))
  (bq-completely-process (read stream t nil t)))
...
(defun bq-attach-append (op item result) 
  (cond ((and (null-or-quoted item)
	...
	;; Until the IDL code that depends on the output of the backquote reader
	;; is killed off, we must decline to do this optimization.  -giba &
	;; jallard, 10/28/97
...
(defun bq-remove-tokens (x) 
  (cond ((eq x *bq-list*) 'bq-list) 
        ((eq x *bq-append*) 'bq-append) 
        ((eq x *bq-nconc*) 'bq-nconc) 
        ((eq x *bq-list**) 'bq-list*) 
        ((eq x *bq-quote*) 'quote) 
        ((atom x) x) 
