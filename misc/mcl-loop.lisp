	     ...
	     (macrolet ((f (overhead &optional (args nil args-p))
	       ...
	       (cond ((setq tem (get fn 'estimate-code-size))
			...
			(setq n (+ n (estimate-code-size-1 (cadr l) env) 1))))
		     ...
		     ((or (#-CCL-2 special-operator-p #+CCL-2 special-form-p fn) (member fn *estimate-code-size-punt*))
...
(defun loop-bind-block ()
  (when (or *loop-variables* *loop-declarations* *loop-wrappers*)
    (push (list (nreverse *loop-variables*) *loop-declarations*
                (loop-make-desetq *loop-desetq-crocks*) *loop-wrappers*)
	  *loop-bind-stack*)
...
(defun loop-do-finally ()
  ...
  (let ((keyword (car *loop-source-code*)))
    (push (if (loop-tequal keyword 'return)
            (progn (loop-pop-source)
                   (loop-construct-return (loop-get-form)))
            (progn
              (when (or (loop-tequal keyword 'do)
                        (loop-tequal keyword 'doing))
                (loop-pop-source))
              (loop-get-progn)))
          *loop-epilogue*)))
...
(defun loop-do-named (restrictive)
  (let ((name (loop-get-form)))
    (unless (symbolp name)
	    (loop-error "The argument ~S to the LOOP NAMED clause must be a symbol"
			name))
    (when restrictive
	  (when *loop-names*
		(loop-error "Can't name LOOP ~S as it is already named ~S"
			    name (first *loop-names*)))
	  (unless (eq *loop-source-code* (cddr *loop-original-source-code*))
		  (loop-error "NAMED must be the first LOOP clause")))
...
(defun loop-for-on (var val data-type)
  (multiple-value-bind (list constantp list-value) (loop-constant-fold-if-possible val)
	...
	(let* ((first-endtest (hide-variable-reference (eq var listvar) listvar `(endp ,listvar)))
...
(defun make-ansi-loop-universe (extended-p)
  (let ((w (make-standard-loop-universe
	     :keywords `((initially (loop-do-initially))
